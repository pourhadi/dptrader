...
(x_train, y_train, x_test, y_test, test_scalers) = get_data('/floyd/input/data/prepped_1Min-5Minx15S.npz', time_val, time_freq_str, seq, None, 15000, False, False, no_high_low=True, npz=True, ahead=ahead, behind=behind) 
...
n = int(x_train.shape[0] / 512) * 512
x_train = x_train[:n]
y_train = y_train[:n]

n = int(x_test.shape[0] / 512) * 512
x_test = x_test[:n]
y_test = y_test[:n]
...
def build_model(layers, d):
    ...
    model.add(GRU(128, input_shape=(layers[0], layers[1]), batch_size=512, stateful=True, return_sequences=True))
    model.add(Dropout(d))
    #model.add(GRU(256, stateful=True, return_sequences=True))
    model.add(GRU(128, stateful=True, return_sequences=False))
